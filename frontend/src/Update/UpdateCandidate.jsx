...
function UpdateCandidate(){
    ...
    const {id} = useParams();
    useEffect(()=>{
        axios.get(`http://localhost:9591/api/getCandidateById/${id}`)
        .then((res)=>{
    ...
    const submitForm = async(e)=>{
        e.preventDefault();
        try {
            const res = await axios.put(`http://localhost:9591/api/updateCandidate/${id}`, candidate);
            console.log("Candidate updated well:", res);
            setMsg(`Successfully updated ${Fname} ${Lname}!`);
            setMsgType("well");
        ...
        <div className="min-h-screen flex justify-center items-center bg-gradient-to-br from-blue-50 to-indigo-100 pt-20 pb-8">
            <div className="w-full max-w-2xl bg-white p-6 shadow-2xl rounded-xl">
                <div className="mb-6 border-b-4 border-indigo-600 pb-4">
                    <h1 className="text-3xl font-bold text-gray-800">Edit Candidate</h1>
                    <p className="text-gray-600 mt-2">Update {Fname} {Lname}'s information</p>
                </div>
