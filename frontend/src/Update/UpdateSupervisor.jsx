...
function UpdateSupervisor(){
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get(`http://localhost:9591/api/sup/${id}`);
                setSupervisorData(res.data);
                console.log("Supervisor found:", res.data);
            } catch (error) {
    ...
    const submitForm = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.put(`http://localhost:9591/api/sup/${id}`, supervisorData);
            console.log("Supervisor updated well:", res);
            setMsg(`Successfully updated ${SupervisorName}!`);
            setMsgType("well");
            setTimeout(() => { navigate("/GetSupervisor"); }, 3500);
        } catch (error) {
            console.log("Failed to update supervisor:", error);
            setMsg(`Update failed: ${error.response?.data?.msg || error.message}`);
            setMsgType("not");
        ...
        <div className="min-h-screen flex justify-center items-center bg-gradient-to-br from-blue-50 to-indigo-100 pt-20 pb-8">
            <div className="w-full max-w-2xl bg-white p-6 shadow-2xl rounded-xl">
                <div className="mb-6 border-b-4 border-indigo-600 pb-4">
                    <h1 className="text-3xl font-bold text-gray-800">Edit Supervisor</h1>
                    <p className="text-gray-600 mt-2">Update {SupervisorName}'s information</p>
                </div>
